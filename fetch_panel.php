...
if (mysqli_num_rows($result) > 0) {
    while ($data = mysqli_fetch_assoc($result)) {
        ...
        $output .= "</td>";
        $output .= "<td class='text-center'>";
        $output .= "<button class='btn btn-primary edit-panel btn-sm' data-id='{$data['id']}' data-company='{$data['company']}' data-email='{$data['email']}' data-manager='{$data['focal_person']}' data-contact='{$data['company_contact']}' data-manager_contact='{$data['focal_person_contact']}' data-province_id='{$data['province_id']}' data-city_id='{$data['city_id']}' data-area_id='{$data['area_id']}' data-status='{$data['status']}' data-services='{$data['services']}'>Edit</button>";
        $output .= "</td>";
        $output .= "<td class='text-center'>";
