...
if (mysqli_num_rows($check_id_exists) > 0) {
    ...
    if ($execute) {
        ...
        $date = date('Y-m-d');
        $time = date('h:i:s');
        $insert_history = mysqli_query($connection, "INSERT INTO tbl_history (page_name,changes_person,change_type,date,time)VALUES('patients','$c_person','enable_patients','$date','$time')");
